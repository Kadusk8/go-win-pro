...
const COLUMNS = [
  { key: 'A fazer',      label: 'A fazer',       color: 'bg-slate-100 text-slate-700',   dot: 'bg-slate-400' },
  { key: 'Em andamento', label: 'Em andamento',   color: 'bg-blue-50 text-blue-700',      dot: 'bg-blue-500' },
  { key: 'Concluído',    label: 'Concluído',      color: 'bg-green-50 text-green-700',    dot: 'bg-green-500' },
];
...
export default function KanbanPage() {
  ...
  const colIdx = (key: string) => COLUMNS.findIndex((c) => c.key === key);
    ...
    <div className="p-8 max-w-7xl mx-auto h-full overflow-y-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Organize as tarefas da equipe</p>
        </div>
        <button
              ...
              <div key={col.key} className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-3 min-h-[120px]">
                  {colTasks.length === 0 ? (
                    ...
                    colTasks.map((task) => {
                      const member = Array.isArray(task.team_members)
                        ? task.team_members[0]
                        : task.team_members;
                      return (
                        <div key={task.id} className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 hover:border-brand-300 transition-colors group">
                          <div className="flex items-start justify-between gap-2 mb-2">
                            ...
                            {deleteId === task.id ? (
                              ...
                              <button onClick={() => setDeleteId(task.id)} className="opacity-0 group-hover:opacity-100 transition-opacity text-slate-300 hover:text-red-500 cursor-pointer shrink-0">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            {member ? (
                              <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-xs text-slate-500 truncate max-w-[80px]">{member.name}</span>
                              </div>
                            ) : (
                              <span className="text-xs text-slate-300 italic">Sem responsável</span>
                            )}

                            <div className="flex gap-1">
                              {colIdx(task.status) > 0 && (
                                <button
                              ...
                              {colIdx(task.status) < COLUMNS.length - 1 && (
                                <button
